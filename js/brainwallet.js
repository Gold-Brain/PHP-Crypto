(function($){
	...
	var input = document.getElementById('input')
	  input.addEventListener(
		...
		function (event) {
		  ...
		  document.getElementById('pass').value = document.getElementById('result').value
      selectedCrypto = document.getElementById('crypto').value
		  // document.getElementById('pass').click()
		  // jQuery.data(document.getElementById('pass'), 'lastvalue', '')
			// document.getElementById('pass').t
		  // document.getElementById('pass').change()
		  onChangePass()
		}
	  )
      ...
      document.getElementById('crypto').addEventListener(
        'click',
        function (event) {
          event.preventDefault()
          document.getElementById('pass').value = document.getElementById('result').value
    ...
    function generateEthereum(address_url_prefix) {
      var hash_str = pad($('#hash').val(), 64, '0');
      var hash = Crypto.util.hexToBytes(hash_str);
      try {
        ...
        $('#genAddrURL').attr('onclick', '');
        $('#genAddrURL').attr('title', addr);
        $('#moneroSec').css('display', 'none');
    ...
    function generateSolana(address_url_prefix) {
      var hash_str = pad($('#hash').val(), 64, '0');
      var hash = Crypto.util.hexToBytes(hash_str);
      try {
        const keyPair = solanaWeb3.Keypair.fromSeed(Uint8Array.from(hash));
        const addr = JSON.parse(JSON.stringify(keyPair.publicKey));
        const sec = JSON.parse(JSON.stringify(bs58_encode(keyPair.secretKey.slice(0, 32))));

        $('#addr').val(addr);
        $('#sec').val(sec);
  
        var qrCode = qrcode(3, 'L');
        var text = $('#addr').val();
        text = text.replace(/^[\s\u3000]+|[\s\u3000]+$/g, '');
        qrCode.addData(text);
        qrCode.make();
    ...
    function generateMonero(address_url_prefix) {
      var hash_str = pad($('#hash').val(), 64, '0');
      var hash = Crypto.util.hexToBytes(hash_str);
      try {
        var keyPair = cnUtil.create_address(hash_str);
        var privKey = mn_encode(hash_str, "english");
        var pubKey = cnUtil.pubkeys_to_string(keyPair.spend.pub, keyPair.view.pub);
        const addr = JSON.parse(JSON.stringify(pubKey));
        const sec = JSON.parse(JSON.stringify(privKey));

        $('#addr').val(addr);
        $('#secViewKey').val(keyPair.view.sec);
        $('#secSpendKey').val(keyPair.spend.sec);
        $('#secMnemonic').val(sec);
        $('#moneroSec').css('display', 'block');
        $('#sec').css('display', 'none');
  
        var qrCode = qrcode(9, 'H');
        var text = $('#addr').val();
        text = text.replace(/^[\s\u3000]+|[\s\u3000]+$/g, '');
        qrCode.addData(text);
        qrCode.make();
        ...
        $('#genAddrQR').html(qrCode.createImgTag(4));
        $('#genAddrURL').attr('href', '#');
        $('#genAddrURL').attr('onclick', 'return false');
        $('#genAddrURL').attr('title', addr);
    ...
    function generateXrp() {
      var hash_str = pad($('#hash').val(), 64, '0');
      var hash = Crypto.util.hexToBytes(hash_str);
      try {
        var account = new ripple.RippleAPI();
        var keyPair = account.generateAddress({entropy: hash, algorithm: 'ecdsa-secp256k1'});
    ...
    function generateZcash() {
      var hash_str = pad($('#hash').val(), 64, '0');
      var hash = Crypto.util.hexToBytes(hash_str);
      try {
        const keyPair = new bitcore.PrivateKey(hash_str);
        const sec = keyPair.toWIF();
        const addr = keyPair.toAddress();
    ...
    function genOnChangeCompressed() {
      console.log("////genOnChangeCompressed");
        setErrorState($('#hash'), false);
        setErrorState($('#sec'), false);
    ...
    function onChangePass() {
        ...
        switch (selectedCrypto) {
          case 'Bitcoin':
            ...
            break;
          case 'Ethereum':
            timeout = setTimeout(generateEthereum('https://etherscan.io'), TIMEOUT);
            break;
          case 'Avalanche':
            timeout = setTimeout(generateEthereum('https://etherscan.io'), TIMEOUT);
            break;
          case 'Solana':
            timeout = setTimeout(generateSolana('https://solscan.io'), TIMEOUT);
            break;
          case 'Monero':
            timeout = setTimeout(generateMonero('https://blockchair.com/monero'), TIMEOUT);
            break;
          case 'BNB':
            timeout = setTimeout(generateEthereum('https://etherscan.io'), TIMEOUT);
            break;
          ...
          case 'Sologenic':
            ...
            break;
          case 'XRP':
            timeout = setTimeout(generateXrp, TIMEOUT);
            break;
          case 'Zcash':
            timeout = setTimeout(generateZcash, TIMEOUT);
            break;
          default:
            timeout = setTimeout(generateEthereum('https://etherscan.io'), TIMEOUT);
            break;
    ...
    function genOnChangePrivKey() {
      console.log("////genOnChangePrivKey");
    ...
    function chOnChangeFormat() {
      console.log("////chOnChangeFormat");
        chMode = $(this).attr('id');
        chUpdate();
    }

    function chAddrToCSV(i, r) {
      console.log("////chAddrToCSV");
        return i + ', "' + r[0] +'", "' + r[1] +'"\n';
    }
    ...
    function chOnChangeBackup() {
      console.log("////chOnChangeBackup");
        clearTimeout(timeout);
        timeout = setTimeout(chUpdateBackup, TIMEOUT);
    }

    function chOnRandom() {
      console.log("////chOnRandom");
    ...
    function txChangeType() {
      console.log("////txChangeType");
        txType = $(this).attr('id');
    }

    function txChangeFrom() {
      console.log("////txChangeFrom");
      txFrom = $(this).attr('id');
